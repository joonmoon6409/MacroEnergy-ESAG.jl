struct BECCSAmmonia <: AbstractAsset
    id::AssetId
    beccs_transform::Transformation
    biomass_edge::Edge{<:Biomass}
    ammonia_edge::Edge{<:Ammonia}
...
function default_data(t::Type{BECCSAmmonia}, id=missing, style="full")
    if style == "full"
...
function full_default_data(::Type{BECCSAmmonia}, id=missing)
...
function simple_default_data(::Type{BECCSAmmonia}, id=missing)
    return OrderedDict{Symbol,Any}(
        :id => id,
        :location => missing,
        :can_expand => true,
...
function make(asset_type::Type{BECCSAmmonia}, data::AbstractDict{Symbol,Any}, system::System)
    id = AssetId(data[:id])
    ...
    nh3_start_node = beccs_transform
    @end_vertex(
        nh3_end_node,
        nh3_edge_data,
        Ammonia,
    ...
    beccs_transform.balance_data = Dict(
        :nh3_production => Dict(
            nh3_edge.id => 1.0,
            biomass_edge.id => get(transform_data, :nh3_production, 0.0)
        ),
    ...
    return BECCSAmmonia(id, beccs_transform, biomass_edge,nh3_edge,elec_edge,co2_edge,co2_emission_edge,co2_captured_edge) 
end
