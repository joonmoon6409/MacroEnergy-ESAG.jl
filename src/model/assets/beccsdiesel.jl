struct BECCSDiesel <: AbstractAsset
    id::AssetId
...
function default_data(t::Type{BECCSDiesel}, id=missing, style="full")
    if style == "full"
...
function full_default_data(::Type{BECCSDiesel}, id=missing)
...
function simple_default_data(::Type{BECCSDiesel}, id=missing)
    return OrderedDict{Symbol,Any}(
        :id => id,
        :location => missing,
        :can_retire => true,
...
function make(asset_type::Type{BECCSDiesel}, data::AbstractDict{Symbol,Any}, system::System)
    id = AssetId(data[:id])
    ...
    return BECCSDiesel(id, beccs_transform, biomass_edge,diesel_edge,elec_edge,co2_edge,co2_emission_edge,co2_captured_edge) 
end