...
function default_data(t::Type{BECCSEthanol}, id=missing, style="full")
    if style == "full"
...
function full_default_data(::Type{BECCSEthanol}, id=missing)
...
function simple_default_data(::Type{BECCSEthanol}, id=missing)
    return OrderedDict{Symbol,Any}(
        :id => id,
        :location => missing,
        :can_expand => true,
...
function make(asset_type::Type{BECCSEthanol}, data::AbstractDict{Symbol,Any}, system::System)
    id = AssetId(data[:id])
