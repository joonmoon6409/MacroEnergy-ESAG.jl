struct BECCSCharcoal <: AbstractAsset
    id::AssetId
    beccs_transform::Transformation
    biomass_edge::Edge{<:Biomass}
    charcoal_edge::Edge{<:Charcoal}
...
function default_data(t::Type{BECCSCharcoal}, id=missing, style="full")
    if style == "full"
...
function full_default_data(::Type{BECCSCharcoal}, id=missing)
    return OrderedDict{Symbol,Any}(
        ...
        :edges => Dict{Symbol, Any}(
            :elec_edge => @edge_data(:commodity => "Electricity"),
            :charcoal_edge => @edge_data(:commodity => "Charcoal"),
...
function simple_default_data(::Type{BECCSCharcoal}, id=missing)
    return OrderedDict{Symbol,Any}(
        :id => id,
        :location => missing,
        :can_expand => true,
...
function make(asset_type::Type{BECCSCharcoal}, data::AbstractDict{Symbol,Any}, system::System)
    id = AssetId(data[:id])
    ...
    @start_vertex(biomass_start_node, biomass_edge_data, commodity, [
        (biomass_edge_data, :start_vertex),
        (data, :location),
    ])
    ...
    charcoal_edge_key = :charcoal_edge
    @process_data(charcoal_edge_data, data[:edges][charcoal_edge_key], [
        (data[:edges][charcoal_edge_key], key),
        (data[:edges][charcoal_edge_key], Symbol("charcoal_", key)),
        (data, Symbol("charcoal_", key)),
    ])
    charcoal_start_node = beccs_transform
    @end_vertex(charcoal_end_node, charcoal_edge_data, Charcoal, [
        (charcoal_edge_data, :end_vertex),
        (data, :location)
    ])
    charcoal_edge = Edge(
        Symbol(id, "_", charcoal_edge_key),
        charcoal_edge_data,
        system.time_data[:Charcoal],
        Charcoal,
        charcoal_start_node,
        charcoal_end_node,
    )
    ...
    @start_vertex(co2_start_node, co2_edge_data, CO2, [
        (co2_edge_data, :start_vertex),
        (data, :co2_sink),
        (data, :location),
    ])
    ...
    @end_vertex(co2_emission_end_node, co2_emission_edge_data, CO2, [
        (co2_emission_edge_data, :end_vertex),
        (data, :co2_sink),
        (data, :location),
    ])
    ...
    @start_vertex(elec_start_node, elec_edge_data, Electricity, [
        (elec_edge_data, :start_vertex),
        (data, :location),
    ])
    ...
    @end_vertex(co2_captured_end_node, co2_captured_edge_data, CO2Captured, [
        (co2_captured_edge_data, :end_vertex),
        (data, :location),
    ])
    ...
    beccs_transform.balance_data = Dict(
        :charcoal_production => Dict(
            charcoal_edge.id => 1.0,
            biomass_edge.id => get(transform_data, :charcoal_production, 0.0)
        ),
