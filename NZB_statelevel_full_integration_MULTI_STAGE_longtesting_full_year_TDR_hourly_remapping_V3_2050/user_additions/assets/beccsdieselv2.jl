struct BECCSDieselv2 <: AbstractAsset
    id::AssetId
...
function default_data(t::Type{BECCSDieselv2}, id=missing, style="full")
    if style == "full"
...
function full_default_data(::Type{BECCSDieselv2}, id=missing)
...
function simple_default_data(::Type{BECCSDieselv2}, id=missing)
    return OrderedDict{Symbol,Any}(
        :id => id,
        :location => missing,
        :can_retire => true,
...
function make(asset_type::Type{BECCSDieselv2}, data::AbstractDict{Symbol,Any}, system::System)
    id = AssetId(data[:id])
    ...
    coinput_commodity_symbol = Symbol(coinput_edge_data[:commodity])
    coinput_commodity = commodity_types()[coinput_commodity_symbol]
    @start_vertex(
        coinput_start_node,
        coinput_edge_data,
        coinput_commodity,
    ...
    coinput_end_node = beccs_transform
    coinput_edge = Edge(
        Symbol(id, "_", coinput_edge_key),
        coinput_edge_data,
        system.time_data[coinput_commodity_symbol],
        coinput_commodity,
        coinput_start_node,
        coinput_end_node,
    )
    ...
    return BECCSDieselv2(id, beccs_transform, biomass_edge,coinput_edge,diesel_edge,elec_edge,co2_edge,co2_emission_edge,co2_captured_edge) 
end